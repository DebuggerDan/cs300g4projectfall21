            ...
            if valid number
                checks if manager flag - isManager(int user_id)
                    if number is manager
                        ...
                        displayWelcome(int display_code);
...
Main Menu Screen:
    Display menu:
        1 - Enter Member Service Entry (must go through process of checking member number validity, 
                                        then enter service code, creates provider and member entry, returns to 
                                        main menu)
            addServiceEntry(int member_id, int logged_in, int service_id, String date_provided); //provider enters in 
                                                                                            //member service provided
        ...
        3 - Request Provider Info (queries provider database by name or number, 
                                    then displays info, then returns to main menu)
            displayProvider(int provider_id); //displays provider info based on provider id (if manager, will 
                                                                                        //display that they are manager)
    ...
    Interactive Mode Menu (IAM Menu):
        1 - Add Member (adds new member to database, takes member name and address, 
            ...
            displayMember(int member_id); //displays member information and provides newly created member id 
                                          //number to user
            
        2 - Delete Member (Deletes member by name or number, requires confirmation, removes member 
                          from database entirely)
        ...
        3 - Modify Member Info (modifies existing member's name and address, cannot modify member number)
            modifyMember(int member_id); //searches for member entry to modify
                ...
                //I would be down to add more methods that modified the services provided entries for each member 
        ...
        5 - Delete Provider (deletes provider by name or number, cannot delete provider that is logged in,
            ...
            removeProvider(String name); //deletes provider based on name
                ^^^ will not delete if provider_id == 000000000 OR provider_id == logged_in OR 
                    checkManager(provider_id) == True
        ...
        modifyProvider(int provider_id); //searches for provider entry to modify
                ...
                //I would be down to add more methods that modified the services provided entries for each provider 
                //but it's not "required"
        ...
        7 - Update Member Statuses (checks member accounts for fees and compare to amounts paid, if not zero, 
            ...
            setStatus(int member_id, int new_status); //changes member to given status 1 - Active, 2 - Suspended,
                                                      //3 - Inactive
            setStatus(); //goes through all members, checks (fees) - (paid), if result == 0, then status = 1, 
                         //else status = 2
    ...
    Generate Reports Menu (Manager Only):
        1 - Member Summary Report (lists member's name, number, address, 
            ...
            displayMemberReport(int member_id); //displays member name and address, all services received 
            (service name, code, date received, fee amount per line), and total services received and total fee to be 
            paid for the week
        ...
        2 - Provider Summary Report (lists provider's name, number, address, 
            ...
            displayProviderReport(int provider_id); //displays provider's name and address, all services provided 
            (service name, code, date provided, and fee per line), and total services provided and total fee to receive
            for the week
        ...
        3 - Accounts Payable/Manager Summary Report (lists every provider to be paid for the week,
                                                      ...
                                                      for an EFT file)
        displayManagerReport(); //displays per line, each provider, their provider id number, the total number of 
        services provided, the total fee amount, and at the bottom it totals all services provided and all fees to be 
        paid out
