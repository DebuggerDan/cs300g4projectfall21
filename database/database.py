...
class Database:
    ...
    @staticmethod
    def get_all_providers(choice, testdate):
        con = sqlite3.connect(path)
        cur = con.cursor()

        if choice == 0: # If we are using the current date, e.g. db.get_all_providers(0, "696969420")
            now = datetime.now()
            thedate = now.strftime("%Y-%m-%d")
            prevweek = datetime.date('thedate', '-6 days')

        if choice == 1: # If we are using a CUSTOM date, e.g. db.get_all_providers(1, "2021-12-02")
            thedate = testdate
            prevweek = datetime.date('thedate', '-6 days')


        #if choice == 1: # If you put in a "1" for the first parameter, e.g. get_all_providers(1, "19")
        cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date >= ? AND Service_Date <= ?", (thedate, prevweek))

        # cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date = ?", (datetime.date('thedate', '-1 day')))
        # cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date = ?", (datetime.date('thedate', '-2 days')))
        # cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date = ?", (datetime.date('thedate', '-3 days')))
        # cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date = ?", (datetime.date('thedate', '-4 days')))
        # cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date = ?", (datetime.date('thedate', '-5 days')))
        # cur.execute("SELECT COUNT(Provider) FROM billing WHERE Service_Date = ?", (datetime.date('thedate', '-6 days')))
        
        #cur.execute("SELECT COUNT(Provider) FROM billing")
    ...
    def get_provider_billing(provider_id):
        ...
        cur.execute("SELECT COUNT(Provider) FROM billing ", (provider_id,))
